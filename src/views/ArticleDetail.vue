	...
	<div class="container">
		...
		<div class="w-layout">
			<div class="w-left">
				<div class="t-top">
					<div class="card-top">
						<div class="in-layout-row"  v-for="(article,index) in articles.slice(0,1)" :key="index">
						    <img :src="article.article.thumbnail" />
						</div>
					</div>
					<div class="card-middle">
						<div class="in-middle"  v-for="(article,index) in articles.slice(0,1)" :key="index">
						    <p>{{article.article.summary}}</p>
						</div>
					</div>
					<div class="card-bottom">
						<img src="../assets/img/YM.png">
					</div>
				</div>
				<div class="t-bo">
					<div class="in-re" v-for="(user,index) in users.slice(8,9)" :key="index">
					    <img :src="user.avatar">
						<div class="i-te">
							<p>{{user.nickname}}</p>
						</div>
					</div>
					<div class="in-publish">
						<img src="../assets/img/36.png">
						<input class="ww" placeholder="添加评论">
						<button class="in-button">评论</button>
					</div>
				</div>
			</div>
			<div class="w-right">
				<div class="in-ava">
					<div class="in-aa" v-for="(user,index) in users.slice(8,9)" :key="index">
						<img :src="user.avatar">
						<div class="ie-text">
							<p>{{user.nickname}}</p>
							<p>{{user.address}}</p>
